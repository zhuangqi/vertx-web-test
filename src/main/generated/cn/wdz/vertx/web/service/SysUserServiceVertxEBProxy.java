...
public class SysUserServiceVertxEBProxy implements SysUserService {
  private Vertx _vertx;
  private String _address;
  private DeliveryOptions _options;
  private boolean closed;

  public SysUserServiceVertxEBProxy(Vertx vertx, String address) {
    this(vertx, address, null);
  }

  public SysUserServiceVertxEBProxy(Vertx vertx, String address, DeliveryOptions options) {
    this._vertx = vertx;
  ...
  public  void create(SysUser user, Handler<AsyncResult<SysUser>> handler){
    ...
    _vertx.eventBus().<JsonObject>request(_address, _json, _deliveryOptions, res -> {
      if (res.failed()) {
        ...
        handler.handle(Future.succeededFuture(res.result().body() == null ? null : new SysUser(res.result().body())));
      }
    });
  }
  @Override
  ...
  public  void getByAccount(String account, Handler<AsyncResult<SysUser>> handler){
    ...
    JsonObject _json = new JsonObject();
    _json.put("account", account);

    DeliveryOptions _deliveryOptions = (_options != null) ? new DeliveryOptions(_options) : new DeliveryOptions();
    _deliveryOptions.addHeader("action", "getByAccount");
    _vertx.eventBus().<JsonObject>request(_address, _json, _deliveryOptions, res -> {
      if (res.failed()) {
        handler.handle(Future.failedFuture(res.cause()));
      } else {
